...
const fetchPokemon = async (id: number): Promise<PokemonMainInfo> => {
  const { data } = await axios.get<PokemonMainInfo>(
...
const fetchPokemons = async (
  ...
  pageSize: number,
  total: number,
): Promise<PokemonMainInfo[]> => {
  const pokemonPromises = [];

  for (
    let id = page * pageSize - pageSize + 1;
    id <= page * pageSize && id < total;
    id++
  ) {
...
const fetchPokemonDescription = async (
  ...
  const { data } = await axios.get<PokemonDescriptionResponse>(
